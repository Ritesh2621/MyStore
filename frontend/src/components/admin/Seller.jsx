...
import {
  Search, ChevronDown, ChevronUp, ShoppingBag, Users, Package,
} from 'lucide-react';
...
const Seller = () => {
  ...
  useEffect(() => {
    if (sellers.length > 0) {
      const productCount = sellers.reduce((sum, s) => sum + (s.productCount || 0), 0);
      const orderCount = sellers.reduce((sum, s) => sum + (s.orderCount || 0), 0);
      const revenue = sellers.reduce((sum, s) => sum + (s.revenue || 0), 0);
      setStats({
        totalSellers: sellers.length,
        totalProducts: productCount,
        totalOrders: orderCount,
  ...
  const fetchSellerProducts = async (sellerId) => {
    ...
    try {
      ...
      const data = await response.json();
      setSellerProducts(Array.isArray(data) ? data : data.products || []);
    } catch (err) {
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <main className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <StatCard title="Total Sellers" value={stats.totalSellers} icon={<Users className="h-6 w-6 text-blue-500" />} color="blue" />
          <StatCard title="Total Products" value={stats.totalProducts} icon={<Package className="h-6 w-6 text-green-500" />} color="green" />
          <StatCard title="Total Orders" value={stats.totalOrders} icon={<ShoppingBag className="h-6 w-6 text-purple-500" />} color="purple" />
          <StatCard title="Total Revenue" value={`₹${stats.totalRevenue.toLocaleString()}`} icon={<span className="text-2xl text-yellow-500">₹</span>} color="yellow" />
        </div>
        ...
        <div className="bg-white p-4 rounded shadow mb-6">
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              {sellerPerformanceData.length > 0 ? (
                <BarChart data={sellerPerformanceData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="name" />
                  <YAxis />
                  <Tooltip />
                ...
                <div className="h-full w-full flex items-center justify-center text-gray-500">No data available</div>
              )}
            </ResponsiveContainer>
          </div>
        </div>

        {/* Seller List & Detail (Mobile Stack / Desktop Side-by-Side) */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Seller List */}
          <div className="bg-white p-4 rounded shadow">
            <div className="flex justify-between items-center mb-4">
              ...
              <input
                type="text"
                placeholder="Search sellers..."
                className="border rounded px-3 py-1 w-full lg:w-auto"
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
              />
            </div>
                ...
                <div className="mt-4">
                  ...
                  <ul className="list-disc pl-5">
                    {sellerProducts.length ? (
                      sellerProducts.map((product) => <li key={product._id}>{product.name}</li>)
                    ) : (
                      <li className="text-gray-400 italic">No products found</li>
                    )}
                  </ul>
                </div>
                ...
                <div className="mt-4">
                  ...
                  <ul className="list-disc pl-5">
                    {sellerOrders.length ? (
                      ...
                      <li className="text-gray-400 italic">No orders found</li>
                    )}
                  </ul>
                </div>
              </>
            ) : (
              <div className="text-gray-500 italic">Select a seller to view details.</div>
            )}
          </div>
        </div>
      </main>
