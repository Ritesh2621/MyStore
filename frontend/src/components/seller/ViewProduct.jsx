...
const ViewProduct = () => {
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2 md:p-4 overflow-y-auto">
        <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-screen overflow-auto">
          <div className="flex justify-between items-center px-4 py-3 md:px-6 md:py-4 border-b border-gray-200">
            <h3 className="text-base md:text-lg font-semibold text-gray-900">Order #{order.tokenNumber || order._id.substring(0, 8)}</h3>
            <button 
              onClick={onClose}
              className="text-gray-500 hover:text-gray-700 focus:outline-none"
            >
          ...
          <div className="px-4 py-3 md:px-6 md:py-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              ...
              <div className="space-y-3 md:space-y-4">
                <div>
                  <h4 className="text-sm font-medium text-gray-500">Order Details</h4>
                  <div className="mt-2 bg-gray-50 p-3 md:p-4 rounded-md">
                    <div className="grid grid-cols-2 gap-2">
                      <div className="text-xs md:text-sm text-gray-500">Order Date</div>
                      <div className="text-xs md:text-sm font-medium">{new Date(order.orderDate).toLocaleString()}</div>
                      
                      <div className="text-xs md:text-sm text-gray-500">Order Status</div>
                      <div>
                        <span className={`px-2 py-1 text-xs rounded-full ${
                          order.orderStatus.toLowerCase() === 'delivered' || order.orderStatus.toLowerCase() === 'completed'
                            ? 'bg-green-100 text-green-800'
                      ...
                      <div className="text-xs md:text-sm text-gray-500">Total Amount</div>
                      <div className="text-xs md:text-sm font-medium">${order.totalAmount.toFixed(2)}</div>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="mt-2 bg-gray-50 p-3 md:p-4 rounded-md">
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <div className="text-xs md:text-sm font-medium">{order.customer.email}</div>
                      
                      <div className="text-xs md:text-sm text-gray-500">Phone</div>
                      <div className="text-xs md:text-sm font-medium">{order.customer.phone}</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3 md:space-y-4">
                <div>
                  ...
                  <div className="mt-2 bg-gray-50 p-3 md:p-4 rounded-md">
                    <div className="space-y-1">
                      ...
                      <p className="text-xs md:text-sm">{order.customer.city}, {order.customer.state} {order.customer.pincode}</p>
                      <p className="text-xs md:text-sm">{order.customer.country}</p>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  <h4 className="text-sm font-medium text-gray-500">Actions</h4>
                  <div className="mt-2 space-y-2">
                    <button className="w-full py-2 bg-white border border-gray-300 text-xs md:text-sm text-gray-700 rounded hover:bg-gray-50 transition">
                      Contact Customer
                    </button>
                  </div>
                </div>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-3 py-2 md:px-6 md:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Qty</th>
                    ...
                    {order.products.map((product, index) => (
                      <tr key={index}>
                        <td className="px-3 py-2 md:px-6 md:py-4 whitespace-nowrap">
                          <div className="flex items-center">
                            <div className="flex-shrink-0 h-8 w-8 md:h-10 md:w-10 bg-gray-200 rounded-md flex items-center justify-center">
                              {product.image ? 
                                <img src={product.image} alt={product.title} className="h-8 w-8 md:h-10 md:w-10 rounded-md" /> : 
                                <span className="text-xs text-gray-500">No img</span>
                              }
                            </div>
                            <div className="ml-2 md:ml-4">
                              <div className="text-xs md:text-sm font-medium text-gray-900">{product.title}</div>
                            </div>
                          </div>
                        </td>
                  ...
                  <tfoot className="bg-gray-50">
                    <tr>
                      <td colSpan="3" className="px-3 py-2 md:px-6 md:py-3 text-right text-xs md:text-sm font-medium text-gray-500">Total</td>
                      <td className="px-3 py-2 md:px-6 md:py-3 text-xs md:text-sm font-medium text-gray-900">${order.totalAmount.toFixed(2)}</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
          ...
          <div className="px-4 py-3 md:px-6 md:py-4 bg-gray-50 border-t border-gray-200 flex justify-end">
            <button 
              onClick={onClose}
              className="px-3 py-1 md:px-4 md:py-2 bg-gray-200 text-xs md:text-sm text-gray-700 rounded hover:bg-gray-300 transition"
            >
              Close
            </button>
          </div>
  ...
  const Pagination = ({ currentPage, totalPages, onPageChange, onPrevPage, onNextPage }) => {
    ...
    return (
      <div className="flex items-center justify-between px-3 py-2 md:px-4 md:py-3 bg-white border-t border-gray-200">
        <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
          <div>
          ...
          <div>
            <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
              <button
                onClick={onPrevPage}
                disabled={currentPage === 1}
                className={`relative inline-flex items-center px-1 py-1 md:px-2 md:py-2 rounded-l-md border border-gray-300 bg-white text-sm font-medium ${
                  currentPage === 1 
                    ? 'text-gray-300 cursor-not-allowed' 
                    : 'text-gray-500 hover:bg-gray-50'
                }`}
              ...
              <div className="flex md:hidden">
                <span className="relative inline-flex items-center px-2 py-2 border border-gray-300 bg-white text-xs font-medium text-gray-700">
                  {currentPage} / {totalPages}
                </span>
              </div>
              ...
              <button
                onClick={onNextPage}
                disabled={currentPage === totalPages}
                className={`relative inline-flex items-center px-1 py-1 md:px-2 md:py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium ${
                  currentPage === totalPages 
                    ? 'text-gray-300 cursor-not-allowed' 
                    : 'text-gray-500 hover:bg-gray-50'
                }`}
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-64">
        <div className="text-base md:text-lg font-medium text-gray-500">Loading dashboard data...</div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="bg-red-50 p-3 md:p-4 rounded-lg border border-red-200">
        <h3 className="text-sm md:text-base text-red-800 font-medium">Error Loading Dashboard</h3>
        <p className="text-xs md:text-sm text-red-600">{error}</p>
        <button 
          className="mt-2 px-3 py-1 md:px-4 md:py-2 bg-red-600 text-white text-xs md:text-sm rounded-md hover:bg-red-700"
          onClick={() => window.location.reload()}
        >
          Retry
        </button>
  ...
  return (
    <div className="space-y-4 md:space-y-6 p-2 md:p-0">
      <h1 className="text-xl md:text-2xl font-bold text-gray-800">Seller Dashboard</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4">
        <div className="bg-white p-3 md:p-6 rounded-lg shadow-sm border border-gray-100">
          ...
          <p className="text-lg md:text-3xl font-bold text-gray-800">{stats.totalOrders}</p>
        </div>
        ...
        <div className="bg-white p-3 md:p-6 rounded-lg shadow-sm border border-gray-100">
          <p className="text-xs md:text-sm font-medium text-gray-500">Pending Orders</p>
          <p className="text-lg md:text-3xl font-bold text-gray-800">{stats.pendingOrders}</p>
        </div>
        
        <div className="bg-white p-3 md:p-6 rounded-lg shadow-sm border border-gray-100">
          <p className="text-xs md:text-sm font-medium text-gray-500">Delivered Orders</p>
          <p className="text-lg md:text-3xl font-bold text-gray-800">{stats.deliveredOrders}</p>
        </div>
      </div>
      ...
      <div className="bg-white p-3 md:p-6 rounded-lg shadow-sm border border-gray-100">
        <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-4 space-y-2 md:space-y-0">
          <h2 className="text-base md:text-lg font-semibold text-gray-800">Sales Overview</h2>
          <div className="flex space-x-1 md:space-x-2">
            <button 
              className={`px-2 py-1 text-xs md:text-sm rounded-md ${timeframe === 'weekly' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-600'}`}
              onClick={() => setTimeframe('weekly')}
            >
              Weekly
            </button>
            <button 
              className={`px-2 py-1 text-xs md:text-sm rounded-md ${timeframe === 'monthly' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-600'}`}
              onClick={() => setTimeframe('monthly')}
            >
              Monthly
            </button>
            <button 
              className={`px-2 py-1 text-xs md:text-sm rounded-md ${timeframe === 'all' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-600'}`}
              onClick={() => setTimeframe('all')}
            >
              All Time
            </button>
      ...
      {/* Recent Orders Table */}
      <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
        <div className="px-3 py-3 md:px-6 md:py-4 border-b border-gray-100">
    ...
    <div className="divide-y divide-gray-200">
      {currentOrders.map(order => (
        <div key={order._id} className="p-3 hover:bg-gray-50">
          ...
          <div className="mb-2">
            <div className="text-xs font-medium">{order.products[0].title}</div>
            <div className="text-xs text-gray-500">
              {`${order.customer.firstName} ${order.customer.lastName}`}
            </div>
          </div>
          ...
          <div className="flex justify-between items-center">
            <div className="text-xs font-medium">₹ {order.totalAmount.toFixed(2)}</div>
            <button
